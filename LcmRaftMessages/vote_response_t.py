...
class vote_response_t(object):
    ...
    def encode(self):
        buf = BytesIO()
        buf.write(vote_response_t._get_packed_fingerprint())
    ...
    def decode(data):
        if hasattr(data, 'read'):
            ...
            buf = BytesIO(data)
        if buf.read(8) != vote_response_t._get_packed_fingerprint():
            raise ValueError("Decode error")
        return vote_response_t._decode_one(buf)
    decode = staticmethod(decode)

    def _decode_one(buf):
        self = vote_response_t()
    ...
    def _get_packed_fingerprint():
        if vote_response_t._packed_fingerprint is None:
            vote_response_t._packed_fingerprint = struct.pack(">Q", vote_response_t._get_hash_recursive([]))
        return vote_response_t._packed_fingerprint
